...
class JoinEventButton extends StatefulWidget {
  const JoinEventButton({Key? key, required this.eventId}) : super(key: key);
    ...
    return SizedBox(
      ...
      child: !isJoined
          ? ElevatedButton(
              onPressed: () {
                if (!isJoined) {
                  context.read<EventsBloc>().add(EventJoined(widget.eventId));
                }
          ...
          : const Center(child: Text("You've already joined this event")),
    );
  }
}
